---
name: notebooklm
description: Acceso completo a Google NotebookLM — crear cuadernos, añadir fuentes, generar podcasts, vídeos, infografías, presentaciones, quizzes, mapas mentales, informes y más. Funciona en Claude Code en Mac, Linux y Windows. Activar con /notebooklm, "instala notebooklm", "crea un podcast sobre X", "genera una infografía", "busca en profundidad sobre X", "añade estas fuentes a NotebookLM", "lista mis cuadernos".
---
<!-- notebooklm-py v0.3.4 | merged: portabilidad Mac+Windows + endurecimiento de credenciales -->
...
if [ -f "$HOME/.notebooklm-venv/bin/notebooklm" ]; then
    ...
    echo "NotebookLM no está instalado. Ejecuta el Step 0 (instalación) primero."
    exit 1
fi
...
A partir de aquí, **todos los comandos usan `$NLM`** en lugar de `notebooklm` directamente. Esto garantiza que funcione en Mac, Linux y Windows.

### Verificación de integridad del CLI (antes del primer uso de la sesión)

Antes de ejecutar `$NLM` por primera vez en una sesión, confirmar que el binario es legítimo:

1. Verificar que vive dentro del venv esperado (`~/.notebooklm-venv/...`). Si `$NLM` apunta a otra ubicación, avisar al usuario antes de continuar.
2. Confirmar que el paquete está instalado ahí: `$VENV_PYTHON -m pip show notebooklm-py`
3. Si el binario existe pero `pip show` no lista `notebooklm-py` — **NO ejecutarlo** y avisar al usuario de que el binario podría no ser legítimo.
...
> En Windows, el venv usa `~/.notebooklm-venv/Scripts/` en lugar de `bin/`. Ajustar las rutas de `pip`/`playwright` en consecuencia.
...
Si la autenticación es correcta (cookie SID presente), **endurecer los permisos del archivo de credenciales** (solo Mac/Linux) y limpiar temporales:

```bash
# Restringir permisos del storage_state (solo Unix; en Windows se omite)
if [ "$OS_FAMILY" = "unix" ]; then
    chmod 600 ~/.notebooklm/storage_state.json
fi

# Limpiar archivos temporales del login
python3 -c "
import tempfile, os
...
Confirmar al usuario que NotebookLM está configurado y listo.
...
| `notebooklm` no encontrado | CLI no instalada | Ejecutar el Step 0 |
